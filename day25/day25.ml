...
class constellation init = object
  val mutable elems = init

  method belongs elem =
    let linker = List.find elems ~f:(fun existing_elem ->
  ...
  method merge (constellations: constellation list) =
    List.iter constellations ~f:(fun other_const ->
        ...
        List.iter const_elems ~f:(fun point ->
            elems <- (point :: elems)
          ) ;
...
let rec group_lines points constellations =
  ...
  | [] -> constellations
  | point :: t_points ->
    let belongs_to = List.filter constellations ~f:(fun const ->
        const#belongs point
      ) in
    ...
    | h_const :: t_consts ->
      ...
      let cleaned_constellations = List.filter constellations ~f:(fun const ->
          const#in_use
        ) in
